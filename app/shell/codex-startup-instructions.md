This repo is driven through xmlui-desktop. The canonical worklist gate is carried by codex's `developer_instructions` (top-level in `~/.codex/config.toml`, installed by xmlui-desktop Setup) and enforced at runtime by a `PreToolUse` hook. Read `app/__shell/conventions.md` for the full conventions, including the two-stage proposed → applied → committed flow, approval payload shape, and edge cases.
